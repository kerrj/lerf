...
class LERFModel(NerfactoModel):
    ...
    def get_max_across(
        self, ray_samples, weights, hashgrid_field, scales_shape, preset_scales=None
    ):
        ...
        if preset_scales is not None:
            ...
            scales_list = torch.linspace(
                0.0, self.config.max_scale, self.config.n_scales
            )
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ray_samples, _, ray_samples_list = self.proposal_sampler(
            ray_bundle, density_fns=self.density_fns
        )
        ray_samples_list.append(ray_samples)

        _, outputs, _ = self._get_outputs_nerfacto(ray_samples)
    ...
    def get_outputs_for_camera_ray_bundle(
        ...
        outputs_lists = defaultdict(
            list
        )  # dict from name:list of outputs (1 per bundle)
        ...
        for i in range(0, num_rays, num_rays_per_chunk):
            ...
            outputs_lists["rgb"].append(outputs["rgb"])
        ...
        for output_name, outputs_list in outputs_lists.items():
            if not torch.is_tensor(outputs_list[0]):
                ...
                continue
            outputs[output_name] = torch.cat(outputs_list).view(image_height, image_width, -1)  # type: ignorex
        return outputs
